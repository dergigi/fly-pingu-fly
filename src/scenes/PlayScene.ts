...
export class PlayScene extends Phaser.Scene {
  ...
  private placeJumpGapScenery(): void {
    ...
    let i = 0;
    for (let y = tipY; y <= baseY; y += 50) {
      const t = (y - tipY) / Math.max(1, baseY - tipY);
      // Widen faster near the bottom so the silhouette reads as a mountain.
      const halfW = tipHalfW + (baseHalfW - tipHalfW) * t * t;
      const step = 56 + Math.floor(t * 18);
      for (let x = tipX - halfW; x <= tipX + halfW; x += step) {
        const n = this.forestNoise(x * 2 + y, 20 + (i % 9));
        // Leave gaps so the pile is rocky, not a solid brick.
        if (n < 0.22 + t * 0.08) {
          i += 1;
          continue;
        }
        const scale = 0.38 + n * 0.55 + t * 0.2;
        plant(
          x + (n - 0.5) * 18,
          y + (this.forestNoise(y, 21 + (i % 5)) - 0.5) * 22,
          n > 0.78 ? "geyser" : "rock-cluster",
          scale,
          n > 0.55 ? 2 : 1,
          n > 0.5,
        );
    ...
    // A few larger pieces: tip plugs the gap, base anchors the mountain.
    const anchors = [
      { key: "geyser" as const, x: tipX, y: tipY + 20, scale: 0.9, depth: 3 },
      { key: "rock-cluster" as const, x: tipX - 28, y: tipY + 70, scale: 0.95, depth: 3 },
      ...
      { key: "geyser" as const, x: tipX - 40, y: baseY - 160, scale: 1.05, depth: 2 },
    ];
  ...
  private placeRunoutScenery(): void {
    ...
    const clusters: ReadonlyArray<{
    ...
    }> = [
      {
        ...
        anchorX: 6400,
        props: [
          { key: "lantern-post", dx: -280, scale: 0.58, sink: 110, depth: 14 },
          { key: "igloo", dx: -140, scale: 0.72, sink: 125, depth: 14 },
          { key: "snow-village", dx: 40, scale: 0.95, sink: 140, depth: 14 },
          { key: "snow-storage", dx: 260, scale: 0.68, sink: 128, depth: 14 },
          { key: "wood-pile", dx: 420, scale: 0.55, sink: 118, depth: 13 },
          { key: "hot-spring", dx: 560, scale: 0.64, sink: 132, depth: 13 },
          { key: "lantern-post", dx: 700, scale: 0.56, sink: 108, depth: 14 },
        ],
      },
      {
        ...
        anchorX: 7900,
        props: [
          { key: "rock-cluster", dx: -360, scale: 0.7, sink: 135, depth: 13 },
          { key: "snow-pile", dx: -220, scale: 0.55, sink: 122, depth: 13 },
          { key: "rock-cluster", dx: -70, scale: 0.82, sink: 148, depth: 13 },
          { key: "geyser", dx: 90, scale: 0.72, sink: 130, depth: 13 },
          { key: "watchtower", dx: 260, scale: 1.05, sink: 145, depth: 15 },
          { key: "rock-cluster", dx: 420, scale: 0.65, sink: 138, depth: 13 },
          { key: "snow-pile", dx: 560, scale: 0.58, sink: 124, depth: 13 },
          { key: "rock-cluster", dx: 710, scale: 0.78, sink: 152, depth: 13 },
          { key: "snow-pile", dx: 850, scale: 0.5, sink: 120, depth: 13 },
        ],
      },
    ];
